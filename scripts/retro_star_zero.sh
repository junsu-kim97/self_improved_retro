...
for ((shard=SHARD_FROM; shard<=SHARD_TO; shard+=1)); do
  ...
  --result_folder ./results/retro_star_zero/x${ITER}/multi-step/succ_traj/shard_${shard} \
  --iteration 500 \
...
for ((shard=SHARD_FROM; shard<=SHARD_TO; shard+=1)); do
  ...
  --plan \
  ./retro_star/results/retro_star_zero/x${ITER}/multi-step/succ_traj/shard_${shard}/plan.pkl \
  --save_path \
  ./retro_star/results/retro_star_zero/x${ITER}/multi-step/succ_traj/shard_${shard}/gold.csv \
  --tpl2prod_save_path \
  ./retro_star/results/retro_star_zero/x${ITER}/multi-step/succ_traj/shard_${shard}/templates.dat \
  --cut_off \
  -thr 0.8 \
  --aug_forward \
...
t_list=""
for ((shard=SHARD_FROM; shard<=SHARD_TO; shard+=1)); do
    g_list="${g_list} ./retro_star/results/retro_star_zero/x${ITER}/multi-step/succ_traj/shard_${shard}/gold.csv"
    t_list="${t_list} ./retro_star/results/retro_star_zero/x${ITER}/multi-step/succ_traj/shard_${shard}/templates.dat"
done
wait
...
-t ${t_list} \
--save_path_gold_csv \
./retro_star/results/retro_star_zero/x${ITER}/multi-step/succ_traj/gold.csv \
--save_path_tpl \
./retro_star/results/retro_star_zero/x${ITER}/multi-step/succ_traj/templates.dat
...
--template_path \
./results/retro_star_zero/x${ITER}/multi-step/succ_traj/templates.dat \
--template_path_test \
./results/retro_star_zero/x${ITER}/multi-step/succ_traj/templates.dat \
--template_rule_path \
./one_step_model/template_rules_1.dat \
--model_dump_folder \
./results/retro_star_zero/x${ITER}/one-step/ \
--fp_dim 2048 \
--batch_size 1024 \
--dropout_rate 0.4 \
--learning_rate 0.0001 \
--train_path \
./results/retro_star_zero/x${ITER}/multi-step/succ_traj/gold.csv \
--test_path \
./results/retro_star_zero/x${ITER}/multi-step/succ_traj/gold.csv \
--train_from \
${BACKWARD_MODEL} \
