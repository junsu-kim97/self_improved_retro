...
if [ ! -d "./results/retro_star_value/x${ITER}/multi-step/succ_traj" ]; then
  ...
  mkdir ./results/retro_star_value/x${ITER}/multi-step/succ_traj
fi

for ((shard=SHARD_FROM; shard<=SHARD_TO; shard+=1)); do
  if [ ! -d "./results/retro_star_value/x${ITER}/multi-step/succ_traj/shard_${shard}" ]; then
    mkdir ./results/retro_star_value/x${ITER}/multi-step/succ_traj/shard_${shard}
  fi
done

if [ ! -d "./results/retro_star_value/x${ITER}/one-step" ]; then
