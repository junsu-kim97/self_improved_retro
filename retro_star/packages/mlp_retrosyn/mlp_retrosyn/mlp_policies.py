...
def fetch_teacher_outputs(teacher_model, dataloader, device):
    ...
    for i, (X_batch, y_batch) in enumerate(dataloader):
        ...
        output_teacher_batch = teacher_model(X_batch).data.cpu().numpy()
        teacher_outputs.append(output_teacher_batch)

    return teacher_outputs

def loss_fn_kd(outputs, labels, teacher_outputs, alpha, temp):
    kd_loss = nn.KLDivLoss()(F.log_softmax(outputs/temp, dim=1),
...
def train_one_epoch_lwf(net, train_loader,
                        optimizer,
                        device,
                        # loss_fn,
                        it,
    ...
    # output_teacher_batch_list = fetch_teacher_outputs(teacher_net, train_loader, device)
    ...
    for i, (X_batch, y_batch) in tqdm(enumerate(train_loader)):
        ...
        output_teacher_batch = teacher_net(X_batch)
        # output_teacher_batch = torch.from_numpy(output_teacher_batch_list[i])
        # output_teacher_batch.to(device)

        loss_fn = loss_fn_kd
        loss_v = loss_fn(output_batch, y_batch, output_teacher_batch, alpha, temp)
        loss_v = loss_v.mean()
        # loss_v = loss_fn(y_hat, y_batch)
        loss_v.backward()
        nn.utils.clip_grad_norm_(net.parameters(), max_norm=5)
...
def train(net, data,
    ...
    if not lwf:
        ...
        old_model.to(device)
        # net = DataParallelModel(net, device_ids=[0,1])
        # old_model = DataParallelModel(old_model, device_ids=[0,1])
    ...
    for e in it:
        # Iterate batches
        if lwf:
            train_one_epoch_lwf(net,train_loader,optimizer,device,it,old_model,alpha,temp)
        else:
...
def load_csv(path, prod_to_rules, template_rules, test=False, backward=True):
    loc = -1 if backward else 0
    X, y = [], []
    df = pd.read_csv(path)
    num = len(df)
    ...
    for i in tqdm(range(num)):
        ...
        product = rxn.strip().split('>')[loc]
        if rule not in prod_to_rules[product]: continue
        # if test and rule not in list(template_rules.keys()): continue
        X.append(product)
        y.append(template_rules[rule])
    return X, y


def train_mlp(prod_to_rules, prod_to_rules_test, template_rule_path, fp_dim=2048,
              batch_size=1024,
              lr=0.001,
              epochs=100,
              weight_decay=0,
    ...
    if train_from != '':
        checkpoint = torch.load(train_from, map_location='cpu')
    ...
    # train_path = '../data/uspto_all/proc_train_cano_smiles_w_tmpl.csv'
    # test_path = '../data/uspto_all/proc_test_cano_smiles_w_tmpl.csv'
    X_train, y_train = load_csv(train_path, prod_to_rules, template_rules, backward=backward)
    X_test, y_test = load_csv(test_path, prod_to_rules_test, template_rules, test=True, backward=backward)
    ...
    old_rollout = None
    if lwf:
        old_rollout = RolloutPolicyNet(n_rules=len(template_rules),fp_dim=fp_dim,dropout_rate=dropout_rate)
        ...
        old_rollout.load_state_dict(old_state_dict)
